...
class ParametersFixed():
    def __init__(self, therapy):
        ...
        if therapy == Therapies.NONE:
            self._prob_matrix = Data.TRANSITION_MATRIX_ANTICOAGULATION
        else:
    ...
    for s in HealthStats:
        ...
        if s == HealthStats.POST_STROKE:
            ...
            prob_matrix[s.value][HealthStats.STROKE.value]\
                = Data.RR_STROKE*Data.TRANSITION_MATRIX_ANTICOAGULATION[s.value][HealthStats.STROKE.value]
            # post-stroke to death
            prob_matrix[s.value][HealthStats.DEATH.value] \
                = Data.RR_STROKE * Data .RR_BLEEDING * Data.TRANSITION_MATRIX_ANTICOAGULATION[s.value][HealthStats.DEATH.value]
            ...
            prob_matrix[s.value] = Data.TRANSITION_MATRIX_ANTICOAGULATION[s.value]
...
def calculate_prob_matrix_combo(matrix_mono, combo_rr):
    """
    :param matrix_mono: (list of lists) transition probability matrix under no therapy
